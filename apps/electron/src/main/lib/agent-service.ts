...
export function rebindActiveAgentStreams(webContents: WebContents): AgentActiveSessionSnapshot[] {
  ...
  // Only a normal main renderer may recover orphaned streams after a reload. Auxiliary
  // windows must never become stream targets merely by reporting their visible tab.
  if (!ownerWindow || ownerWindow !== getMainWindow()) return []

  const rebound: AgentActiveSessionSnapshot[] = []
  for (const snapshot of orchestrator.listActiveSessionSnapshots()) {
    // Preserve live routes. Rebinding them on every tab visibility update lets a second
    // renderer steal another window's stream; only routes whose previous target died need recovery.
    if (streamRoutes.get(snapshot.sessionId)) continue
    streamForwarder.clear(snapshot.sessionId)
    streamRoutes.rebind(snapshot.sessionId, webContents)
    rebound.push(snapshot)
  }
  if (rebound.length > 0) attachWebContentsCleanup(webContents)
  return rebound
}
