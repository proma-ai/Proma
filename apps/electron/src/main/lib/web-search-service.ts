...
/** WebFetch 目前只由用户显式配置的 Tavily key 支持；Cloud 代理仅提供 WebSearch。 */
export function isWebFetchEnabledForAgent(): boolean {
  return getToolState('web-search').enabled && !isCloudWebSearchEnabled() && !!getTavilyApiKey()
}
...
export async function searchWeb(options: WebSearchOptions): Promise<TavilySearchResponse> {
  ...
  if (isCloudWebSearchEnabled()) {
    const token = getAuthToken()
    if (!token) throw new Error('云端搜索失败：未登录')

    const { baseUrl } = getCloudApiConfig()
    const response = await fetchWithTimeout(`${baseUrl}/tools/web-search`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => '')
      throw new Error(`云端搜索请求失败 (${response.status}): ${errorText || response.statusText}`)
    }
