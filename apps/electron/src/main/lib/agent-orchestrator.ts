...
export class AgentOrchestrator {
    ...
    if (channel.provider === 'proma') {
      ...
      const run = async (token: string): Promise<string | null> => {
        const request = getAdapter('proma').buildTitleRequest({
          ...
          apiKey: token,
          modelId: PROMA_TITLE_MODEL,
          prompt: TITLE_PROMPT + userMessage,
        })
        return fetchTitle(request, getAdapter('proma'), getFetchFn(await getEffectiveProxyUrl()))
      }
      try {
        ...
        console.warn('[Agent 标题生成] Proma Cloud 标题生成失败:', error)
        return null
      }
    }

    if (channel.provider === 'xai' || channel.provider === 'github-copilot') {
      // Subscription providers use Pi's provider-specific OAuth transport; the generic channel
      // adapter only understands API keys, so retain a local deterministic title.
  ...
  async sendMessage(
    ...
    // 2.1 获取渠道凭据可能含 await；重新检查并在首次用户消息持久化前原子抢占。
    // 这样并发请求即使同时通过 preflight，也不会重复删除 retry error 或写入同一用户消息。
    if (this.activeSessions.has(sessionId)) {
      console.warn(`[Agent 编排] 会话 ${sessionId} 已在 preflight 期间启动，拒绝重复请求且不保存用户消息`)
      callbacks.onError(getActiveRunRejectionMessage())
      completeBeforeRun()
      return
    }

    // 用户在 preflight 期间已点击停止：不再启动本轮，也不落盘用户消息。
    if (this.stoppedBeforeRunSessions.has(sessionId)) {
      completeBeforeRun({ stoppedByUser: true })
      return
    }
    ...
    if (shouldPersistInitialUserMessage({ hasActiveRun: false, retryOfErrorUuid })) {
      try {
        ...
        callbacks.onError(`消息保存失败：${message}`, { runGeneration })
        callbacks.onComplete([], { startedAt: streamStartedAt, runGeneration })
        return
      }
    }
    ...
    const selectedOfficialAgentModel = channel.provider === 'proma'
      ? (channel.agentModels ?? channel.models).find((candidate) => candidate.id === selectedModelId)
      : undefined
    const piBaseUrl = channel.provider === 'proma'
            ...
            if (msg.type === 'assistant' && !isPartialMessage) {
              ...
              if (assistantMsg.error) {
                ...
                if (hasPiPartialOutput) {
                  ...
                  partialOutput._channelId = channel.id
              ...
              if (!msgRecord.isReplay && !isPartialMessage) {
                if (msg.type === 'user') {
                  ...
                  if (msg.type === 'assistant') {
                    ...
                    assistantRecord._channelId = channel.id
