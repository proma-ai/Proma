...
import type { AgentSendInput, AgentEvent, AgentMessage, AgentGenerateTitleInput, AgentProviderAdapter, AgentSessionMeta, TypedError, RetryAttempt, SDKMessage, SDKAssistantMessage, SDKResultMessage, AgentStreamPayload, RewindSessionResult, SdkBeta, ProviderType } from '@proma/shared'
import {
      ...
      for (let attempt = 1; attempt <= MAX_AUTO_RETRIES + 1; attempt++) {
        ...
        try {
                ...
                if (typedError.code === 'invalid_api_key' && channel.provider === 'proma' && attempt <= MAX_AUTO_RETRIES) {
                  ...
                  try {
                    ...
                    if (newToken) {
                      ...
                      lastRetryableError = typedError.message
          ...
          try {
            ...
            const errorCode = isBillingError
              ? 'billing_error'
              : isPromptTooLong
                ? 'prompt_too_long'
                : isThinkingSignature
                  ? THINKING_SIGNATURE_ERROR_CODE
                  : 'unknown_error'
            const errorTitle = isBillingError
              ? '余额不足'
              : isPromptTooLong
                ? '上下文过长'
                : isThinkingSignature
                  ? THINKING_SIGNATURE_ERROR_TITLE
                  : '执行错误'
            const errorContent = isBillingError
