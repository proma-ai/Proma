...
import {
  ...
  resolvePiApiKey,
  stripLegacyAgentSdkContextSuffix,
...
describe('Pi runtime 模型 ID [1m] 剥离', () => {
  test('Given 带 [1m] 后缀的模型 ID When strip Then 剥离后缀', () => {
    expect(stripLegacyAgentSdkContextSuffix('glm-5.2[1m]')).toBe('glm-5.2')
  })
  ...
  test('Given 无后缀模型 ID When strip Then 原样返回', () => {
    expect(stripLegacyAgentSdkContextSuffix('glm-4.6')).toBe('glm-4.6')
  })

  test('Given [1m] 出现在中间(非结尾) When strip Then 不剥离', () => {
    expect(stripLegacyAgentSdkContextSuffix('foo[1m]-bar')).toBe('foo[1m]-bar')
  })

  test('Given undefined When strip Then 返回 undefined', () => {
    expect(stripLegacyAgentSdkContextSuffix(undefined)).toBeUndefined()
  })
})
