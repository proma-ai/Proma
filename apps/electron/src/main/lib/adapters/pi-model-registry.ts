...
function resolvePromaOfficialApi(
  ...
  if (leafModelId.startsWith('gpt-')) return 'openai-responses'
  if (
    leafModelId.startsWith('claude-')
    || leafModelId.startsWith('deepseek-')
    || leafModelId.startsWith('glm-')
  ) {
    return 'anthropic-messages'
  }
...
export async function resolvePiReasoningCapability(
  ...
  const profile = resolveReasoningProfile({
    ...
    transport: provider === 'openai-codex' || provider === 'xai'
      ? 'openai-responses'
      : toReasoningTransport(normalizePiApi(provider, resolvedModelId, modelApiProtocol)),
  })
...
async function resolvePiModelDefaults(input: PiAgentQueryOptions): Promise<PiModelDefaults> {
  ...
  const api = normalizePiApi(input.provider, input.model, input.modelApiProtocol)
  ...
  return {
    ...
    // 仅把 Claude Adaptive Thinking 所需的安全 compat 标记带入临时 provider；
    // 其它 catalog compat 不能跨渠道或跨协议继承。
...
function normalizePiBaseUrl(baseUrl: string | undefined, provider: ProviderType, api: Api): string | undefined {
  ...
  if (provider === 'proma') {
    const rootUrl = baseUrl
      ...
      .replace(/\/api\/v\d+$/, '')
      .replace(/\/v\d+$/, '')
    return api === 'anthropic-messages' ? rootUrl : `${rootUrl}/v1`
  }
