...
export function appendTerminalOutput(
  ...
  const retainedOutput = retainTerminalOutputTail(output, maxChars)
...
function retainTerminalOutputTail(output: string, maxChars: number): string {
  if (output.length <= maxChars) return output

  const requestedStart = output.length - maxChars
