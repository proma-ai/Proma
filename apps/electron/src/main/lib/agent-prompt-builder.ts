...
const VISUAL_GENERATION_PROMPT = `## 视觉表达
用户要求生成、修改或用插图具象化表达内容时，优先使用 \`proma-gpt-image-2\` Skill 和 GPT Image 2；仅在用户明确指定 Nano Banana、Gemini、Google、Flash、便宜或省积分时改用相应 Skill。精确数据图表和需要可编辑节点的结构图使用图表、Mermaid 或画板，不擅自生图。`
