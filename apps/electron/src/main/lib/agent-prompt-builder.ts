...
const VISUAL_GENERATION_PROMPT = `## 商业版视觉表达
GPT Image 2 是 Proma 商业版的默认视觉能力。不要只将它理解为用户明确提出“画图”的工具：在设计、品牌、产品、空间、服装、营销、提案、课程、故事、内容策划或概念解释等任务中，只要一张画面能显著提高理解、说服力、审美完成度或交付可用性，就主动使用 \`proma-gpt-image-2\` Skill 和 GPT Image 2 产出一张与结论/方案配套的视觉素材。用户要求生成、修改或用插图具象化表达内容时同样直接使用；无需先抽象地询问“是否需要配图”。

默认生成一张 medium 质量、与使用场景匹配比例的图片，并在文字交付中说明其用途。把视觉作为解决问题和完成提案的一部分，而不是最后可有可无的装饰；尤其面对设计方向、情绪氛围、空间感、人物/产品场景、风格探索或难以仅靠文字理解的抽象概念时，应优先生成。仅在用户明确指定 Nano Banana、Gemini、Google、Flash、便宜或省积分时改用相应 Skill。

不为简短的纯事实/纯文字问答、用户明确要求不使用图片的任务，或精确数据图表而擅自生成图片；精确数据图表和需要可编辑节点的流程图、架构图、关系图使用图表、Mermaid 或画板。涉及需确认的付费边界时遵循当前权限策略，但不要把确认问题泛化为“要不要视觉化”。`

const BUILTIN_MCP_PROMPT = `## 内置能力
Proma 内置 MCP 工具不会出现在工作区 mcp.json。只要 \`mcp__*\` 工具出现在当前工具列表中即为可用，调用时照抄真实的下划线 server 名；不要因其不在 workspace_state 中而忽略或改用低级替代。`
...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  ...
  const planningPrompt = todosEnabled || calendarEnabled
    ? [
        ...
        '外部来源（`nativeOrigin`）的修改、完成或删除先说明副作用并确认。规划、承诺交付、询问近期安排或结束含行动项的对话时，按需读取已启用的 Todo/日程。',
