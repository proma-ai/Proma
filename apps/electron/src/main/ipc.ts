...
import { listChangelogs } from './lib/changelog-service'
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (event): Promise<import('@proma/shared').GithubCopilotOAuthLoginResult> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_, channelId: string, modelId: string) => {
      ...
      const officialModel = channel.provider === 'proma'
        ? (channel.agentModels ?? channel.models).find((model) => model.id === modelId)
        : undefined
      return resolvePiReasoningCapability(channel.provider, modelId, officialModel?.apiProtocol)
    }
  )
  ...
  ipcMain.handle(
    CHANGELOG_IPC_CHANNELS.LIST,
    async (_, options?: ChangelogListOptions): Promise<ChangelogListResponse> => {
      return listChangelogs(options)
    },
  )
