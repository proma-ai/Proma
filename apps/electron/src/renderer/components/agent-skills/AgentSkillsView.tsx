...
import { buildCatalogMcpGuidePrompt, MCP_INTEGRATION_CATALOG, isCatalogIntegrationVisible, matchesCatalogSearch, type CatalogCliIntegration, type CatalogCliProbeState, type CatalogCredentialIntegration, type CatalogGuidedIntegration, type CatalogMcpIntegration } from './integration-catalog'
...
export function AgentSkillsView({
  ...
  // Only hide entries that are actually visible in the catalog. Existing configurations for
  // temporarily hidden integrations must remain manageable under “我的 MCP”.
  const catalogServerNames = React.useMemo(() => new Set(
    MCP_INTEGRATION_CATALOG
      .filter((integration): integration is CatalogMcpIntegration => integration.kind === 'mcp' && isCatalogIntegrationVisible(integration))
      .map((integration) => integration.serverName),
  ), [])
    ...
    >
      ...
      {embedded && (
        <div className="titlebar-no-drag mx-auto w-full max-w-6xl shrink-0 px-3 pb-3">
          <AgentActionHint action={tab === 'enterprise' ? '浏览、安装或手动更新企业 Skills' : tab === 'skills' ? '创建、整理、更新或删除 Skills' : '查找、配置或移除 MCP'} />
        </div>
      )}
      ...
      <div className={cn('titlebar-no-drag mx-auto flex w-full max-w-6xl shrink-0 items-center gap-3', embedded ? 'px-3 pb-3' : 'px-8 pb-4')}>
        ...
        {!embedded && <div className={cn('relative grid h-8 shrink-0 items-stretch rounded-xl bg-muted p-0.5', enterpriseSkillsEnabled ? 'w-[480px] grid-cols-4' : 'w-[360px] grid-cols-3')}>
          <div
            className={cn(
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {customSkills.length > 0 && (
        <SkillSection title="我的 Skills" skills={customSkills} embedded={embedded} isBuiltin={isBuiltin} updatingSkill={updatingSkill} canPublishToEnterprise={canPublishToEnterprise} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} onPublishToEnterprise={onPublishToEnterprise} />
      )}
      {builtinSkills.length > 0 && (
        <SkillSection title="PROMA 内置" skills={builtinSkills} embedded={embedded} isBuiltin={isBuiltin} updatingSkill={updatingSkill} canPublishToEnterprise={false} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} onPublishToEnterprise={onPublishToEnterprise} />
      )}
    </div>
  )
}
...
function SkillSection({ title, skills, embedded, isBuiltin, updatingSkill, canPublishToEnterprise, onOpen, onToggle, onUpdate, onPublishToEnterprise }: SkillSectionProps): React.ReactElement {
        ...
        <McpSection title="Proma 集成能力" count={builtinServers.length} embedded={embedded}>
          {builtinServers.map((server) => (
            <McpCard
              ...
              entry={{ type: 'stdio', command: 'Proma 运行时注入', enabled: server.enabled, isBuiltin: true }}
