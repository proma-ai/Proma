...
export function WorkspaceMemoryWindowApp(): React.ReactElement {
  ...
  React.useEffect(() => {
    return window.electronAPI.onWorkspaceMemoryWindowOpenFile((relativePath) => {
      if (dirtyRef.current) {
        toast.message('请先保存当前编辑内容，再切换到其他记忆文件。')
        return
      }
