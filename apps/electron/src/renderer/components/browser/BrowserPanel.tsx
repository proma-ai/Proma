...
import { ChevronLeft, ChevronRight, LoaderCircle, PanelTopClose, PictureInPicture2, RotateCw, ShieldAlert, Square } from 'lucide-react'
...
interface BrowserPanelProps {
  ...
  /** 主窗口槽内触发「弹出到独立窗口」。未接入弹出能力的宿主可不传。 */
  onPopOut?: () => void
  /** 本面板正渲染在受管浏览器独立窗口内（隐藏弹出按钮，工具栏由独立窗口标题栏承担）。 */
  inDetachedWindow?: boolean
}

export function BrowserPanel({ sessionId, tabId, state, onPopOut, inDetachedWindow = false }: BrowserPanelProps): React.ReactElement {
  ...
  return (
    <div className={cn('flex h-full min-w-0 flex-col overflow-hidden bg-content-area titlebar-no-drag', !inDetachedWindow && 'border-l border-border/80')}>
      <div className={cn('flex items-center h-[42px] gap-1 px-2 border-b border-border/40 bg-muted/20', !inDetachedWindow && getWindowControlsPaddingClass(isWindows))}>
        ...
        {!inDetachedWindow && onPopOut && (
          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="size-8 rounded-lg text-muted-foreground/70 hover:bg-muted/60 hover:text-foreground" disabled={riskBlocked || !state} onClick={() => void onPopOut()} aria-label="弹出到独立窗口"><PictureInPicture2 className="size-4" /></Button></TooltipTrigger><TooltipContent>弹出到独立窗口</TooltipContent></Tooltip>
        )}
        ...
        {inDetachedWindow && (
          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="size-8 rounded-lg text-muted-foreground/70 hover:bg-muted/60 hover:text-foreground" onClick={() => void dockBack()} aria-label="收起回主窗口"><PanelTopClose className="size-4" /></Button></TooltipTrigger><TooltipContent>收起回主窗口</TooltipContent></Tooltip>
        )}
      </div>
      {riskAcknowledged === true ? (
        <BrowserSlot key={tabId} sessionId={sessionId} tabId={tabId} presentationTarget={inDetachedWindow ? 'detached' : 'main'} />
      ) : (
        <div className="flex flex-1 min-h-0 items-center justify-center bg-muted/15 px-8 text-center">
