...
export function LeftSidebar({ width, noTransition }: LeftSidebarProps): React.ReactElement {
  ...
  // 读取最新 tabs 但不进入依赖：refreshAgentSidebarSessions 被多个 effect 依赖，
  // 若随 tabs 变化重建会让每次开关标签都重新拉取会话列表。
  ...
  const refreshAgentSidebarSessions = React.useCallback(async (includeArchived: boolean): Promise<void> => {
    ...
    const sessions = includeArchived
      ...
      : activeSessions
    // active 刷新的列表不含归档条目，直接覆盖会让仍在 Tab 中打开的归档会话丢失
    // workspace/model 等会话级 metadata。只补回少量被 Tab 引用的归档项。
    setAgentSessions((previous) => {
      ...
      const openArchived = previous.filter((session) => (
        session.archived && openSessionIds.has(session.id) && !nextIds.has(session.id)
      ))
      if (openArchived.length === 0) return sortAgentSessionsByUpdatedAtDesc(sessions)
      return sortAgentSessionsByUpdatedAtDesc([...sessions, ...openArchived])
    })
  ...
  // 初始加载对话列表；用户档案由顶层 UserProfileInitializer 统一维护。
  React.useEffect(() => {
    window.electronAPI
      .listConversations()
      .then((list) => {
...
/** Obsidian 在 Chat 中占用主内容区，在有 Agent 会话时复用右侧项目级工作区。 */
  ...
  const handleOpenCapabilityComponent = React.useCallback((component: 'skills' | 'enterprise' | 'mcp' | 'memory'): void => {
    if (mode !== 'agent' || !currentAgentSessionId) {
