...
import { ChevronDown, ChevronUp, Paperclip, FileText, Sparkles, Server, Download, MessageSquareText, Workflow } from 'lucide-react'
...
type MentionType = 'file' | 'skill' | 'mcp' | 'session' | 'flow'
...
const MENTION_STYLES: Record<MentionType, { icon: typeof FileText; className: string }> = {
  ...
  flow: { icon: Workflow, className: 'bg-[hsl(38_92%_50%/0.16)] text-[hsl(38_92%_38%)]' },
}
...
export function remarkMentions() {
  return (tree: MdastParent) => {
    walkMdastText(tree, (node, index, parent) => {
      ...
      const mentionPattern = /@file:(\S+)|\/skill:(\S+)|#mcp:(\S+)|&session:(\S+)|!flow:(\S+)/g
      ...
      while ((m = mentionPattern.exec(text)) !== null) {
        ...
        const mType: MentionType = m[1] ? 'file' : m[2] ? 'skill' : m[3] ? 'mcp' : m[4] ? 'session' : 'flow'
...
/** mention:// URL 匹配 */
const MENTION_URL_RE = /^mention:\/\/(file|skill|mcp|session|flow)\/(.+)$/
