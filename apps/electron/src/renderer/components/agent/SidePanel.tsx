...
import { X, ExternalLink, ChevronRight, MoreHorizontal, FolderSearch, Pencil, FolderInput, GitBranch, GitMerge, MessageSquarePlus, FileDiff, FileText, FolderOpen, Globe, MessageCircle, Brain, Building2, Split, Blocks, CalendarDays, ListTodo, Clock, ServerCog, SquareTerminal, Terminal } from 'lucide-react'
...
export function SidePanel({ sessionId, sessionPath, activeTab, onTabChange, width = 460 }: SidePanelProps): React.ReactElement {
  ...
  const isWorkspaceComponentRenderable = React.useCallback((component: WorkspaceComponentTab): boolean => (
    isWorkspaceComponentEnabled(component)
    && (component !== 'enterprise' || enterpriseSkills.loading || enterpriseSkills.enabled)
  ), [enterpriseSkills.enabled, enterpriseSkills.loading, isWorkspaceComponentEnabled])
  ...
  const effectiveActiveTab: AgentSidePanelTab = activeTab === 'chat' && !sideChatConversationId
    ...
    : activeTab === 'temporary-agent' || (activeExplorationSessionId !== null && !activeExplorationBranch) || (activeTab === 'delegation' && !selectedDelegationSession) || (activeTerminalId !== null && !terminalTabs.some((terminal) => terminal.terminalId === activeTerminalId))
      ? 'files'
      : isWorkspaceComponentTab(activeTab) && (!workspaceSlug || !workspaceComponentTabs.includes(activeTab) || !isWorkspaceComponentRenderable(activeTab))
  ...
  const workspaceTabs = React.useMemo<WorkspacePanelTab[]>(() => [
    ...
    ...workspaceComponentTabs.filter(isWorkspaceComponentRenderable).map((component) => {
  ...
  ], [activeBrowserTabId, browserState, isWorkspaceComponentRenderable, previewFiles, selectedDelegationSession, selectedDelegationStatus, sessions, sessionId, showBrowserActivity, sideChatConversationId, sideTemporaryAgents, terminalTabs, workspaceComponentTabs])
  ...
  const renderWorkspaceTabContent = (paneTab: AgentSidePanelTab, paneWidth: number): React.ReactNode => {
    if (isWorkspaceComponentTab(paneTab) && !isWorkspaceComponentRenderable(paneTab)) {
      return renderWorkspaceTabContent('files', paneWidth)
    }
    ...
    ) : paneTab === 'enterprise' && (enterpriseSkills.loading || enterpriseSkills.enabled) ? (
        ...
        >
          <DiffPanelTabBar
            ...
            onOpenWorkspaceComponent={(component) => {
              if (!isWorkspaceComponentEnabled(component) || (component === 'enterprise' && !enterpriseSkills.enabled)) return
