...
| 信号类型 | 维护位置 |
|---|---|
| 项目硬规则、架构边界、常用命令、必须遵守的约束 | CLAUDE.md |
| 用户偏好、误判纠正、问题已解决/未解决/加重、跨会话经验 | Memory |
| 重复执行的步骤、固定检查清单、可复用工作流 | Skill |
| 当前任务 todo、plan、handoff、临时研究笔记 | 会话级 Context |
| 跨会话调研报告、设计文档、分析正文、长 checklist | 工作区级 Context / 本地文档 |
| 调 API、查数据、调用外部服务 | Chat 工具或 tool-builder |
...
| **F** | 应该维护 Memory / CLAUDE.md / Context | "记住这个"、"不是这个原因"、"已经解决/还没解决/更严重了"、"以后别再这样判断" |
...
1. **CLAUDE.md**：当前工作目录及附加目录是否已规定相关规则或入口
2. **Memory**：工作区 `.claude/memory/MEMORY.md` 和相关主题文件是否已有偏好、经验或纠错
3. **工作区 Skills**：`~/.proma/agent-workspaces/<workspace>/skills/`
4. **OSS 内置 Skills**：brainstorming / writing-plans / executing-plans / skill-creator / find-skills / tool-builder / guizang-ppt-skill / docx / pdf / pptx / xlsx
5. **会话级 Context**：当前 cwd 下 `.context/` 是否已有本次任务的计划、todo、handoff 或中间结论
6. **工作区级 Context / 本地文档**：工作区 `workspace-files/.context/` 或本地文档是否已有相关调研、设计、决策或长文档
...
- **不要追加冲突记忆**——如果用户纠正了旧结论，要修订或标注旧结论过时，而不是简单新增相反说法
